...
public class COABE {
	...
	//*************************************************************************************************
	public Element[][] genXYVec (int bit_len, int num_att) {
	...
	private static final String FILENAME = "PrivateKey.txt";
	...
	private static final String FILENAME_public = "mpk.txt";
	...
	/* Code to write to a file public parameters*/
	private static final String FILENAME_secret = "msk.txt";
	...
	public static  void main (String[] args) {
		...
		Element[] attri_set = new Element[length];
		BigInteger[] ba = new BigInteger[length];
		
		for(int i =0; i <length; i++){
			ba[i]= new BigInteger(args[i].getBytes());
			Element abc = mZr.newElement().set(ba[i]);
			attri_set[i]=abc;
			System.out.println(ba[i]+"--"+abc);
		}
		
		//Element[] sk = co_abe.genKey(attri_set);
		//String s = "";
		
			/* Code to concatenate the generated key and then convert it to string so that */
			/* it can be written to the text file*/
		/*for(int j=0;j <length;j++){
			//System.out.println("J: "+ j);
			//System.out.println("-----"+sk[j]+"--"+j+"!!!!");
			Element attr_va_el = sk[j];
			System.out.println("-----"+attr_va_el);
			String str_pk = new String(attr_va_el.toString());
			if(j==0){
				s=str_pk;
			}else{
			s = s+"#"+str_pk;
		...
		/*
		   for (int j=0;j<length;j++) {
            ...
            byte[] temp = sk[j].toBytes();
            String temp_str = Base64.getEncoder().encodeToString(temp);
            //if(u==0){
                s=s+temp_str;
            //}else{
              //  s= s+"#"+temp_str;
		 ...
		 Element[] public_param = co_abe.getPubParam();
		 String json_pub = "";
		 for(int l=0;l<9;l++){
			Element pub = public_param[l];
			String pub_k = new String(pub.toString());
			json_pub = json_pub + l + " : \n"+ pub_k + "\n";	
		 }
		 writeToFile_public(json_pub);
		 
		 /****** Generate Master Key**********/
		 Element[] master_secret = co_abe.getMasterSecret();
		 String s_secret = "";
		 for(int m = 0; m <5;m++){
			 Element msecret = master_secret[m];
			 String msec = new String(msecret.toString());
			 s_secret = s_secret + msec;
		 }
		 writeToFile_secret(s_secret);
